...
public class RCC_SceneManager : MonoBehaviour {
	...
	void Update(){
		...
		if (ControlFreak2.CF2Input.GetKeyDown (RCC_Settings.Instance.recordKB))
			RCC.StartStopRecord ();

		if (ControlFreak2.CF2Input.GetKeyDown (RCC_Settings.Instance.playbackKB))
			RCC.StartStopReplay ();

		if (ControlFreak2.CF2Input.GetKey (RCC_Settings.Instance.slowMotionKB))
			Time.timeScale = .2f;

		if (ControlFreak2.CF2Input.GetKeyUp (RCC_Settings.Instance.slowMotionKB))
