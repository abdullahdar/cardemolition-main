...
//		transform.rotation = playerCar.transform.rotation * Quaternion.AngleAxis(10f, Vector3.right);
	...
	private void Inputs(){
		...
		case RCC_Settings.ControllerType.Keyboard:
			...
			if (ControlFreak2.CF2Input.GetKeyDown (RCCSettings.changeCameraKB))
				ChangeCamera ();

			orbitX += ControlFreak2.CF2Input.GetAxis (RCCSettings.mouseXInput) * orbitXSpeed * .02f;
			orbitY -= ControlFreak2.CF2Input.GetAxis (RCCSettings.mouseYInput) * orbitYSpeed * .02f;

			break;

		case RCC_Settings.ControllerType.XBox360One:

			if (ControlFreak2.CF2Input.GetButtonDown (RCCSettings.Xbox_changeCameraKB))
				ChangeCamera ();

			orbitX += ControlFreak2.CF2Input.GetAxis (RCCSettings.Xbox_mouseXInput) * orbitXSpeed * .01f;
			orbitY -= ControlFreak2.CF2Input.GetAxis (RCCSettings.Xbox_mouseYInput) * orbitYSpeed * .01f;

			break;

		case RCC_Settings.ControllerType.PS4:

			if (ControlFreak2.CF2Input.GetButtonDown (RCCSettings.PS4_changeCameraKB))
				ChangeCamera ();

			orbitX += ControlFreak2.CF2Input.GetAxis (RCCSettings.PS4_mouseXInput) * orbitXSpeed * .01f;
			orbitY -= ControlFreak2.CF2Input.GetAxis (RCCSettings.PS4_mouseYInput) * orbitYSpeed * .01f;
	...
	private void TPS(){
		...
		case RCC_Settings.ControllerType.Keyboard:
			...
			if(ControlFreak2.CF2Input.GetKey(RCCSettings.lookBackKB))
		...
		case RCC_Settings.ControllerType.XBox360One:
			...
			if(ControlFreak2.CF2Input.GetButton(RCCSettings.Xbox_lookBackKB))
				wantedRotation = wantedRotation * Quaternion.AngleAxis ((180), Vector3.up);
